...
class MACTelnetProtocol(asyncio.Protocol):
    ...
    def __init__(self, mac, username, password, on_session_end):
        self.stop_command_flag = False  # Variabila pentru a indica dacă trebuie să opriți comanda
    ...
    def handle_control_packet(self, packet):
        if packet.control_packets[0].packet_type == CP_ENCRYPTION_KEY:
            ...
            confirmation.control_packets.append(ControlPacket(CP_PASSWORD, confirmation_code))
            confirmation.control_packets.append(ControlPacket(CP_USERNAME, self.username.encode()))

            term_type = platform.system().encode()
    ...
    def stop_command(self):
        ...
        command_packet.data = b'\x03'  # Modificăm pentru a trimite Ctrl+C
...
async def main(mac: str, username: str, password: str):
    ...
    def on_tab_press(event):
        if event.name == 'tab':
            # Check if there's ongoing user input
            if hasattr(protocol, 'user_input') and protocol.user_input:
                # Send tab character only if there's ongoing user input
                protocol.send_tab()

    keyboard.on_press(on_tab_press)

    signal.signal(signal.SIGINT, signal_handler)#to close  terminal  use CTRL+BREAK
    # Use signal_command_stop function with proper definition
    signal.signal(signal.SIGBREAK, signal_handler) #to stop comand use CTRL+C in thinkpad is FN+B
    ...
    # Move the definition of signal_command_stop before calling it
